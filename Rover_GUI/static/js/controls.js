...
let _bboxTimeout = null;

function drawBoundingBox(det) {
    const canvas = document.getElementById('bboxCanvas');
    if (!canvas) return;
    const img = document.getElementById('cameraFeed');
    // Sync canvas size to the rendered image
    canvas.width  = img.clientWidth  || canvas.offsetWidth;
    canvas.height = img.clientHeight || canvas.offsetHeight;
    ...
    if (_bboxTimeout) clearTimeout(_bboxTimeout);
    _bboxTimeout = setTimeout(() => {
        ctx.clearRect(0, 0, canvas.width, canvas.height);
        _bboxTimeout = null;
    }, 7000);
}
...
function initializeKeyboard() {
    document.addEventListener('keydown', (e) => {
        // If the user is typing in any input or textarea, never intercept.
        // This allows WASD/QE to be typed freely in the NOVA chat box.
        if (e.target.tagName === 'INPUT' || e.target.tagName === 'TEXTAREA') {
            return;
        }

        // Prevent default browser behaviour for control keys (scrolling etc.)
    ...
    document.addEventListener('keyup', (e) => {
        // Mirror the guard so keys don't get stuck in keysPressed set
        if (e.target.tagName === 'INPUT' || e.target.tagName === 'TEXTAREA') {
            return;
        }
